  ...
  <div class="Page Confirm">
    ...
    <table class="GoodList">
      <tbody>
        ...
        <tr class="good">
          <td class="name First">
            <a :href="'https://localhost:8088/course/'+order.courseId" target="_blank">
              <img :src="order.courseCover">
            </a>
            <div class="goodInfo">
              <a :href="'https://localhost:8088/course/'+ order.courseId" target="_blank">{{ order.courseTitle }}</a>
            </div>
          </td>
...
export default {
  ...
  created() {
    orderApi.getById(this.$route.params.id).then(response => {
      ...
      console.log('orderid is', this.order.id)
    })
  },

  methods: {
    toPay() {
      orderApi.updateStatus(this.order.id).then(response => {
        // 支付成功后
        if (response.success) {
